...
object ActorCapabilities extends App {
  ...
  object CounterActor {
    def props(count: Int) = Props(new CounterActor(count))
  }
  class CounterActor(var count: Int) extends Actor {
    override def receive: Receive = {
      case "+" => {
          count += 1
          println(s"[$self] Incremented: count is $count")}
      case "-" => {
          count -= 1
          println(s"[$self] Decremented: count is $count")}
      case "print" => println(s"[$self] Count is $count")
      case _ =>
    }
  }
  val actor1 = system.actorOf(CounterActor.props(0))
  actor1 ! "+"
  actor1 ! "+"
  actor1 ! "-"
  actor1 ! "print"
  ...
  // EXERCISE 1 ROCKTHEJVM SOLUTION
    ...
    override def receive: Receive = {
      case Deposit(amount) =>
        if (amount < 0) sender() ! TransactionFailure("Invalid deposit amount")
          ...
          funds += amount
          sender() ! TransactionSuccess(s"Success, deposited $amount")
        }
      case Withdraw(amount) =>
        if (amount < 0) sender() ! TransactionFailure("Invalid withdraw amount")
          ...
          funds -= amount
          sender() ! TransactionSuccess(s"Success, withdrew $amount")
        }
