...
//  println(account.getAmount)
  ...
  /**
   * rant#3: tracing and dealing with errors in a multithreaded enc is a PAIN IN THE ASS
   */
  // 1 000 000 numbers in between 10 threads
  import scala.concurrent.ExecutionContext.Implicits.global
  val futures = ( 0 to 9 )
    .map(i => 100000 * i until 100000 * (i+1)) // 0 -99999, 100000 - 199999, 200000 - 299999 etc
    .map(range => Future {
      if (range.contains(543329)) throw new RuntimeException("invalid number")
      range.sum
    })
